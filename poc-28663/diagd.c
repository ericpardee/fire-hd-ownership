...
int main(void)
{
	...
	/* diagnostic: reclaim as CPU-readable region, look for markers */
	{
		union kbase_ioctl_mem_alloc ma;
		memset(&ma, 0, sizeof(ma));
		...
		ma.in.va_pages = 256;
		ma.in.commit_pages = 256;
		if (ioctl(fd, KBASE_IOCTL_MEM_ALLOC, &ma) < 0) { perror("diag alloc"); return 1; }
		void *r = mmap(NULL, 256 << PAGE_SHIFT, PROT_READ | PROT_WRITE, MAP_SHARED, fd, ma.out.gpu_va);
		if (r == MAP_FAILED) { perror("diag mmap"); return 1; }
		int hits = 0;
		for (int i = 0; i < 256 * (int)(0x1000 / 8); i++) {
			if (((__u64 *)r)[i] != 0 && (((__u64 *)r)[i] & 0xFFFFFFFF0000ULL) == 0xFEEDFACE0000ULL) hits++;
		}
		printf("[diag] marker qwords in fresh 256-page region: %d\n", hits);
	}
	return 0;
}
